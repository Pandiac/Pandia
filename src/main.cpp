...
#if defined(NDEBUG)
# error "Pandia cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x527d4da7a6f151da7affe5c1d53e2a75c28409ede9bd7d072d023820936775fa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pandia: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pandia Signed Message:\n";
...
{
    // Pandia: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Pandia: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Pandia: 3 days
static const int64 nTargetSpacing = 2 * 60; // Pandia: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pandia: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pandia: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x708f12044ba4a10314aea1e468066f363e74b2997140ca8309963ef659ab0f07");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Pandia 1.10.2023";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045f5fbaa853556b5eaef5ffa6a4718c66e3d0a9cf57164d814d44b14585aa3485b99d81e3929af0c674772ed8c7cadd0798560c52fa25ec7df1eec2da3151ab63") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1695162258;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085929909;

        if (fTestNet)
        {
            block.nTime    = 1695162234;
            block.nNonce   = 385280923;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x41f8b2e8ae73781f15b5e06c97e185956fe400cb0373692146e3d2d2a13a9eb8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb2, 0xda }; // Pandia: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PandiaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PandiaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PandiaMiner(CWallet *pwallet)
{
    printf("PandiaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pandia-miner");
    ...
    try { loop {
        ...
        printf("Running PandiaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PandiaMiner, pwallet));
}
